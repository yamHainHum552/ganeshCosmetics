...
const ClientCard = ({ name, price, productId, work, link, image }) => {
  ...
  const setTrimmedName = (name) => {
    return name.length > 14
      ? name.slice(0, 10).concat("...").toUpperCase()
      : name.toUpperCase();
  };

  return (
    <div
      className="hover:scale-105 gap-1 w-[130px] transition-all ease-in duration-75 flex flex-col md:gap-5 bg-[#f2f2f2] shadow-md shadow-gray-500 text-black md:w-[250px] items-center justify-center rounded-md"
    ...
    >
      ...
      <div className="flex items-center justify-center ">
        <h1 className="font-bold md:text-xl">
          {isHovered ? name.toUpperCase() : setTrimmedName(name)}
        </h1>
      </div>
      {/* Product Price */}
